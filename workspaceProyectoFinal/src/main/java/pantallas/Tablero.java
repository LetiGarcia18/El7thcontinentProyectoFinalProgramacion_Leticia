...
public class Tablero extends JPanel {
	...
	/** Booleano que nos indicará con true si hemos ganado la partida. Se inicializa a false inicialmente **/
	...
	/**
	 * Constructor de Tablero, al cual se le pasa por parámetros la ventana que va a contener el JPanel de Tablero, un ArrayList con las cartas que va 
	 * a haber en el mapa, y el personaje. En este constructor se va a dibujar el tablero. Van a aparecer en la parte central/izquierda las cartas de terreno
	 * iniciales, el token del personaje, en la parte lateral derecha van a aparecer distintos JLabel actuando de separadores entre las cartas de estado del
	 * personaje, el inventario del personaje y las acciones que va a poner ir realizando el personaje
	...
	/**
	 * Función pública que va a dibujar las cartas en el tablero de juego, donde se le van a pasar por parámetros en ancho de la casilla
	 * (que representa el ancho de la carta), el margen izquierdo, y el margen superior. Va a recorrer el ArrayList de las cartasEnMapa 
	 * y va a ir almacenando en la variable X la posición exacta en la que se debe colocar en el tablero, al igual que irá haciendo con la posición Y,
	 * de modo que al dibujarse, no se van a solapar las cartas. 
	 * Va a realizar lo mismo con las cartas de estado, con la peculiaridad de que va a comprobar si la carta de estado esta ya dibujada en el 
	 * tablero, y si es así, calcula la posición de la siguiente carta de estado, para que cuando se dibuje más de una, no se solapen tampoco.
	 * Y por último, realizará lo mismo con las cartas de inventario. Realizará el mismo procedimiento que se realiza con las cartas de estado,
	 * antes de dibujar la carta inventario, comprobará si está en ya dibujada en el tablero, y si es así calculará la posición de la siguiente
	 * para que nuevamente, no se solapen.
	...
	/**
	 * Función pública que va a dibujar individualmente cada carta. Para ello, va a coger la ruta de la imagen de cada carta, la escalará
	 * al tamaño adecuado, y si se trata de cartas de estado, aparecerá un JLabel encima de cada carta de estado con el título 
	 * de los estados. Se le va a pasar por parámetros la carta a dibujar, la posición X e Y de la carta y el ancho de la misma.
	...
	/**
	 * Función pública que va a dibujar al persona en el tablero de juego, donde se le van a pasar por parámetros la carta de terreno donde se va a posicionar
	 * el personaje, el ancho de la casilla de la carta, el tamaño que tendrá el token del personaje, el margen izquierdo que va a el token del personaje y 
	 * el margen superior que va a tener el token del personaje. Esta función va a calcular la posición X e Y de la carta donde el perosnaje se va a situar,
	 * y va a llamar a otra función que dibujará al persona encima de esa carta.
	 ...
	 * @param margenSuperior El margen superior que va a tener el token del personaje
	 */
	...
	/**
	 * Función pública que va a dubujar al personaje, donde se le va a pasar por parámetros la posición X e Y de la carta, el ancho de la carta,
	 * y el tamaño del personaje. Se va a coger la ruta de la imagen del personaje, y se va a escalar al tamaño adecuado que se quiere que tenga el 
	 * token.
	 * Se dibujará también un JLabel en la parte superior izquierda de la pantalla de Tablero, en el que va a mostrar la energía que tiene el personaje
	 * durante el juego.
	 * @param posicionXCarta La posición X de la carta donde se va a encontrar el personaje posicionado
	 * @param posicionYCarta La posición Y de la carta donde se va a encontrar el personaje posicionado
	...
	/**
	 * Función privada que nos va a dar una carta que se encuentre en el mapa, junto a su número de carta. Se le van a pasar por parámetros 
	 * el número de la carta. Esta función va a recorrer el ArrayList de las cartasEnMapa, y nos va a devolver la carta que tenga como
	 * número de carta, el número de carta que se le pasa por parámetros, y si no la encuentra, nos devuelve null.
	 * @param numeroCarta El número de carta que queremos que nos devuelva
	 * @return La carta que coincida con el número de carta que se le pasan por parámetros, o null si no encuentra la carta.
	 */
	...
	/**
	 * Función privada que nos devuelve la carta donde se encuentra el personaje. Se le pasa por parámetros el número de carta.
	 * Esta función, dependiendo del tipo de carta que queramos, va a recorrer un ArrayList u otro. Si el número de carta que le pasamos
	 * por parámetros coincide con alguna carta de estado que tenga ese número, nos devolverá esa carta de estado. Si el número de carta
	 * que le pasamos por parámetros no coincide con ninguna de estado, se metererá a buscar en el ArrayList de las cartas de inventario,
	 * y si la encuentra, nos la devolverá. Y si no encuentra ni un tipo de carta u otra, nos devolverá null.
	 * @param numeroCarta El número de carta que queremos que nos devuelva
	 * @return Nos devolverá o una carta de estado, o una carta de inventario o null si no la encuentra en ninguno de los dos tipos.
	 */
	...
	/**
	 * Función pública que nos va a dibujar las acciones en el tablero. Primero, va a utilizar la funcion dameCartaEnMapaConNumero para ver en qué carta
	 * se encuentra el personaje, y también va a utilizar la función dameCartasAdyacentes , que va a devolver las cartas adyacentes a la carta donde está
	 * posicionado el personaje.
	 * Una vez realizado esto, llamando a sus correspondientes funciones, se van a dibujar las acciones de la carta en la que está posicionado el personaje, y de las 
	 * cartas adyacentes, también, si existen cartas de evento, se dibujarán las acciones de dichas cartas, si el personaje tiene estados, también se dibujarán las acciones
	 * de las cartas de esos estados y si el personaje tiene inventario, se dibujarán las acciones de las cartas de inventario.
	 */
	...
	/**
	 * Función pública que va a dibujar las acciones de cada carta. Se le va a pasar por parámetros la carta de la que se van a dibujar las acciones
	 * y las cartas adyacentes que puedan tener acciones a dibujar. En esta función se iterará por las acciones de la carta y se llamará a la función
	 * dibujarAccion para que se vayan dibujando dichas acciones en el Tablero.
	 * @param carta La carta de la que se van a dibujar las acciones
	 * @param cartasAdyacentes Las cartas adyacentes a la carta de la que se van a dibujar las acciones.
	 */
	...
	/**
	 * Función pública que va a dubujar la acción en el Tablero de juego. Se le va a pasar por parámetros la acción a dibujar, y las cartas adyacentes de la carta
	 * donde se encuentra la acción. En esta función va a comprobar si el ArrayList de cartas adyacentes no está vacío, si no lo está va a ir dibujando las acciones de 
	 * las cartas adyacentes. Si esas acciones (las de las cartas adyacentes) están dibujadas, se desactivará la acción de la carta donde se encuentra el personaje, siendo
	 * imposible realizarla de nuevo.
	 * También se dibujarán las acciones de la carta donde está situado el personaje. Se representarán con un botón donde pondrá la acción de la que se trata, y al lado de 
	 * cada botón, aparecerá el coste y la dificultad que va a tener resolver dicha acción.
	 * Para el caso de la acción MOVE, va a tener al lado del botón, un botón desplegable donde aparecerán el número de todas las cartas de terreno donde va a ser posible desplazarse.
	 * @param accion La acción que se va a dibujar
	 * @param cartasAdyacentes Las cartas adyacentes de la carta donde se encuentra la acción. 
	 */
